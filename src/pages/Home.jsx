...
// 이미지들
import windowBg from "../assets/window_shelf.png"; // 선반 창가 배경
...
const familyCode = "family001";
const plantType = "sunflower"; // "strawberry" or "tomato"
...
function Home() {
  ...
  const fetchPoints = async () => {
    try {
      const data = await getFamilyPoints(familyCode);
  ...
  const handleWater = async () => {
    ...
    setTimeout(async () => {
      try {
        await deductFamilyPoints(familyCode, 100);
